...
/// Parse an unquoted token (until whitespace)
fn unquoted_token<'a>(s: &mut Input<'a>) -> ModalResult<&'a str> {
    // Accept non-empty continuous non-whitespace
    take_while(1.., |c: char| !c.is_whitespace()).parse_next(s)
}
...
pub fn url_parse<'a>(s: &mut Input<'a>) -> ModalResult<Curl<'a>> {
    preceded(multispace0, alt((quoted_data_parse, unquoted_token)))
        .map(|url_str| {
...
pub fn method_parse<'a>(s: &mut Input<'a>) -> ModalResult<Curl<'a>> {
    preceded(
        ...
        (
            multispace0,
            literal("-X"),
            // either space + value or directly attached value (e.g., -XPOST)
            alt((
                (multispace1, alt((quoted_data_parse, unquoted_token))).map(|(_, v)| v),
                unquoted_token,
            )),
        )
        .map(|(_, method, data)| {
...
pub fn header_parse<'a>(s: &mut Input<'a>) -> ModalResult<Curl<'a>> {
    preceded(
        ...
        (
            ...
            literal("-H"),
            multispace1,
            alt((quoted_data_parse, unquoted_token)),
        )
        .map(|(_, header, _, data)| {
...
pub fn data_parse<'a>(s: &mut Input<'a>) -> ModalResult<Curl<'a>> {
    preceded(
        ...
        (
            multispace0,
            // Match longer flags before shorter to avoid prefix capture
            alt((
                literal("--data-urlencode"),
                literal("--data-binary"),
                literal("--data-raw"),
                literal("--data"),
                literal("-d"),
            ...
            alt((
                // allow space then value
                (multispace1, alt((quoted_data_parse, unquoted_token))).map(|(_, v)| v),
                // or equals sign then value (no space)
                (literal("="), alt((quoted_data_parse, unquoted_token))).map(|(_, v)| v),
                // or attached value for short -d (e.g., -dname=John)
                unquoted_token,
            )),
        )
            .map(|(_, data_flag, data)| {
                Curl::Data(CurlStru {
...
/// Parse options that take a value and are not data/method/header, map to Flag with data
pub fn option_with_value_parse<'a>(s: &mut Input<'a>) -> ModalResult<Curl<'a>> {
    preceded(
        opt(slash_line_ending),
        (
            multispace0,
            ...
            alt((
                (multispace1, alt((quoted_data_parse, unquoted_token))).map(|(_, v)| v),
                (literal("="), alt((quoted_data_parse, unquoted_token))).map(|(_, v)| v),
                // attached for short forms like -oFILE, -m10, -xhttp://...
                unquoted_token,
            )),
        )
            .map(|(_, flag, val)| {
...
pub fn flag_parse<'a>(s: &mut Input<'a>) -> ModalResult<Curl<'a>> {
    preceded(
        ...
        (
            ...
            .map(|(_, dash, dash2, body): (_, char, Option<char>, &str)| {
                let mut id = String::new();
                id.push(dash);
                if let Some(d2) = dash2 { id.push(d2); }
                id.push_str(body);
                Curl::Flag(CurlStru { identifier: id, data: None })
            }),
    )
    .parse_next(s)
}
...
mod tests {
    ...
    #[rstest]
    #[case(" https://example.com", "https")]
    #[case("http://test.com/path", "http")]
    fn test_url_parse_unquoted(#[case] input: String, #[case] expected: &str) {
        let mut input = LocatingSlice::new(input.as_str());
        let result = url_parse(&mut input);
        assert!(result.is_ok());
        if let Ok(Curl::URL(url)) = result {
            // Just check we parsed a URL with expected schema
            use crate::url::protocol::Schema;
            let schema = match expected { "https" => Schema::HTTPS, _ => Schema::HTTP };
            assert_eq!(url.schema, schema);
        } else {
            panic!("Expected URL variant");
        }
    }
    ...
    #[rstest]
    #[case(r#" --data-raw '{"a":1}' "#, "--data-raw", "{\"a\":1}")]
    #[case(r#" --data-binary @file.bin "#, "--data-binary", "@file.bin")]
    #[case(r#" --data-urlencode key=value "#, "--data-urlencode", "key=value")]
    fn test_data_parse_extended(
        #[case] input: String,
        #[case] expected_identifier: String,
        #[case] expected_data: String,
    ) {
    ...
    #[case(" -k ", "-k")]
    #[case("-L", "-L")]
    #[case(" --retry-all-errors ", "--retry-all-errors")]
