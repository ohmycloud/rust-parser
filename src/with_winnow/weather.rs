...
pub fn parse_kv_pair<'a>(input: &mut &'a str) -> PResult<Kv<'a>> {
    let mut parsed_key = preceded(space0, take_until(1.., "="))
        .map(|x: &str| x.trim());
    let mut parsed_value = preceded(space0, till_line_ending)
        .map(|x: &str| x.trim());
    seq!(
        Kv {
            key: parsed_key,
...
pub fn parse_observation<'a>(input: &mut &'a str) -> PResult<HashMap<String, Vec<f64>>> {
    ...
    hash.insert(format!("{}", parsed_year), parse_temperatures.parse_next(input).unwrap());
    Ok(hash)
}
...
mod tests {
    ...
    End year= 2009"#;
        ...
        assert_eq!(expected, vec![
            Kv {key: "Name", value: "Jan Mayen"},
            Kv {key: "Country", value: "NORWAY"},
            Kv {key: "Lat", value: "70.9"},
            Kv {key: "Long", value: "8.7"},
            Kv {key: "Height", value: "10"},
            Kv {key: "Start year", value: "1921"},
            Kv {key: "End year", value: "2009"}
        ])
    }
    ...
    fn test_parse_observations() {
        ...
        hash1.insert("1921".into(), vec![
            -4.4, -7.1, -6.8, -4.3, -0.8, 2.2, 4.7, 5.8, 2.7, -2.0, -2.1, -4.0
        ]);
        let mut hash2: HashMap<String, Vec<f64>> = HashMap::new();
        hash2.insert("2009".into(), vec![
